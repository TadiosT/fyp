...
from campus_occupancy.cv.lecture_cv import (  # noqa: E402
    detect_people,
    draw_boxes,
    get_video_info,
    load_homography,
...
from campus_occupancy.simulation.seat_snap import build_seat_tree, load_seats, snap_detections  # noqa: E402
from campus_occupancy.simulation.seating import assess_zones, zone_shapes  # noqa: E402
...
SEAT_CSV_PATH = "data/lecture_144_seats.csv"

# Match Huxley's STATE_COLOURS for the same visual language across the app.
STATE_COLOURS = {"In Use": "#32CD32", "Empty": "#FF0000"}
...
if not os.path.exists(SEAT_CSV_PATH):
    st.error(
        f"🚨 Seat map not found at `{SEAT_CSV_PATH}`. "
        "Click each seat once to generate it:\n\n"
...
ss.setdefault("lh_occupied_seats", set())
ss.setdefault("lh_latest_detections", [])
ss.setdefault("lh_snap_debug", [])
...
with ctrl_m:
    if st.button("↻ Restart", use_container_width=True):
        ...
        ss.lh_occupied_seats = set()
        ss.lh_latest_detections = []
        ss.lh_snap_debug = []
        ss.lh_zones = None
        ss.lh_recommended = None
        ss.lh_tick_count = 0
with ctrl_r:
...
def _render_recommendation():
    ...
    if is_first:
        st.success(f"👉 **Sit in the {name}** rows ({free} of {cap} seats free).")
    else:
        prev = zones[zones.index(recommended) - 1]["name"]
        st.warning(f"⚠️ {prev} is full. Try the **{name}** ({free} of {cap} seats free).")
...
def _render_map():
    """Plotly floor plan with every seat plotted, coloured by occupancy."""
    seats = load_seats(SEAT_CSV_PATH).copy()
    occupied = ss.lh_occupied_seats
    seats["state"] = seats["seat_id"].map(
        lambda sid: "In Use" if sid in occupied else "Empty"
    )

    shapes = zone_shapes(ss.lh_recommended) if ss.lh_recommended is not None else None

    fig = build_floor_map(
        image=load_floor_plan(),
        points_df=seats,
        color_col="state",
        color_map=STATE_COLOURS,
        category_order=["In Use", "Empty"],
        custom_data_cols=["seat_id", "state"],
        hover_template=(
            "<b>%{customdata[0]}</b><br>"
            "%{customdata[1]}"
            "<extra></extra>"
        ),
        marker={"size": 9, "opacity": 0.85, "line": {"width": 1, "color": "white"}},
        legend_title="Seat state",
        height=700,
        extra_shapes=shapes,
    )
...
def _render_debug_expander():
    """Cropped feed with YOLO overlays + per-detection snap diagnostics."""
    with st.expander("🔍 Debug: cropped feed + snap diagnostics", expanded=False):
        ...
        st.image(
            rgb,
            caption=f"Frame {ss.lh_frame_idx:,}, cropped PiP",
            use_container_width=True,
        )
        if ss.lh_snap_debug:
            st.caption("Per-detection snap result (closer = better):")
            st.dataframe(
                pd.DataFrame(ss.lh_snap_debug),
                use_container_width=True,
                hide_index=True,
            )
...
def cv_tick():
    """Advance video + run YOLO + snap to seats + render. Fragment-scoped rerun."""
    seats = load_seats(SEAT_CSV_PATH)
    tree = build_seat_tree(seats)
    ...
    if ss.lh_playing:
        ...
        if crop is not None:
            ...
            # Foot point (bbox bottom-centre) lies on the floor plane the
            # homography models.
            feet = [((x1 + x2) / 2.0, y2)
                    for (_, _, _, (x1, y1, x2, y2)) in detections]
            H, dst_size = load_homography(HOMOGRAPHY_PATH)
            projected = project_to_floor_plan(feet, H, dst_size=dst_size)

            occupied, debug = snap_detections(projected, seats, tree)
            ss.lh_occupied_seats = occupied
            ss.lh_snap_debug = debug
            ss.lh_latest_detections = detections
            ss.lh_tick_count += 1

        zones, recommended = assess_zones(seats, ss.lh_occupied_seats)
        ss.lh_zones = zones
        ss.lh_recommended = recommended

    total = len(seats)
    used = len(ss.lh_occupied_seats)
    st.subheader(f"Seats occupied: {used} / {total}")
